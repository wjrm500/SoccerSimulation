...
class Club:
    def __init__(self, league, city):
        ...
        self.players = []
        numPlayersPerClub = (
            league.system.universe.config["numPlayersPerClub"]
            or config.systemConfig["numPlayersPerClub"]
        )
    ...
    def getProvisionalShortName(self, precedence=0):
        ...
        ### Etc.
        firstLetterAndConsonantsOnly = self.name[0] + re.sub(r"(?i)[aeiou -\.,]", "", self.name[1:])
        provisionalShortName = (
            firstLetterAndConsonantsOnly[: precedence + 3]
            if len(firstLetterAndConsonantsOnly) >= precedence + 3
            else self.name[:3]
        ).upper()
    ...
    def getShortName(self):
        ...
        ### getProvisionalShortName method
        ...
        selfRankAmongClashingClubs = sorted(
            clashingClubs, key=lambda x: x.getRating(), reverse=True
        ).index(self)
    ...
    def getMatchReports(self, gameweek=None):
        matchReports = [
            matchReport for matchReport in self.league.matchReports if self in matchReport["clubs"]
        ]
        if gameweek is None:
            return matchReports
