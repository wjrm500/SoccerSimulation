...
class Team:
    ...
    def getSelectOffenceDefence(self, select):
        ...
        for skill, value in player.skillDistribution.items():
            selectOffence += (
                (value * selectRating)
                / 30
                * config.matchConfig["contribution"][skill]["offence"]
                * positionOffenceDefence["offence"]
            )
            selectDefence += (
                (value * selectRating)
                / 30
                * config.matchConfig["contribution"][skill]["defence"]
                * positionOffenceDefence["defence"]
            )
    ...
    def getSelectOffensiveDefensiveContribution(self, select):
        selectOffence, selectDefence = (
            self.selectionOffences[select],
            self.selectionDefences[select],
        )
        teamOffence, teamDefence = self.offence, self.defence
        return {
            "offensive": selectOffence / teamOffence,
            "defensive": selectDefence / teamDefence,
        }
    ...
    def setSelectionOffensiveDefensiveContributions(self):
        ...
        for select in self.selection:
            selectOffensiveDefensiveContribution = self.getSelectOffensiveDefensiveContribution(
                select
            )
            self.selectionOffensiveContributions[select] = selectOffensiveDefensiveContribution[
                "offensive"
            ]
            self.selectionDefensiveContributions[select] = selectOffensiveDefensiveContribution[
                "defensive"
            ]
    ...
    def setGoalFactors(self):
        ...
        for select in self.selection:
            ...
            goalFactor = (
                (
                    (
                        (player.skillDistribution["offence"] * 2)
                        + player.skillDistribution["technique"]
                    )
                    / 3
                )
                * selectRating
                * config.matchConfig["goalLikelihood"][position]
            ) ** 2
            goalFactors[player] = goalFactor
        sumGoalFactors = sum(goalFactors.values())
        self.goalFactors = {
            player: goalFactor / sumGoalFactors for player, goalFactor in goalFactors.items()
        }
    ...
    def setAssistFactors(self):
        ...
        for select in self.selection:
            ...
            assistFactor = (
                (
                    (
                        (
                            (player.skillDistribution["spark"] * 2)
                            + player.skillDistribution["technique"]
                        )
                        / 3
                    )
                    * selectRating
                    * config.matchConfig["assistLikelihood"][position]
                )
                ** 1.5
        ...
        self.assistFactors = {
            player: assistFactor / sumAssistFactors
            for player, assistFactor in assistFactors.items()
        }
    ...
    def getGoalScorer(self):
        goalscorer = np.random.choice(
            list(self.goalFactors.keys()), p=list(self.goalFactors.values())
        )
        return goalscorer
    ...
    def getGoalAssister(self):
        ...
        assister = np.random.choice(
            list(self.assistFactors.keys()), p=list(self.assistFactors.values())
        )
        return assister
